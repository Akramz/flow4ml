    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Computing errors"):
            ...
            for i in range(len(x)):
                ...
                # Sample metadata
                metadata = {key: batch[key][i] for key in batch if key not in ["image", "target"] and isinstance(batch[key], list)}
...
def analyze_errors(error_data, n_worst=20):
    ...
    sorted_indices = sorted(range(len(errors)), key=lambda i: errors[i]["mae"], reverse=True)
...
def visualize_worst_samples(analysis_results, output_dir):
    ...
    for i, sample in enumerate(worst_samples):
        ...
        if worst_metadata and len(worst_metadata) > i:
            ...
            plt.figtext(0.5, 0.01, metadata_str, ha="center", fontsize=10, 
...
def create_error_histograms(analysis_results, output_dir):
    ...
    plt.xlabel("MAE")
    plt.ylabel("Count")
    plt.axvline(df["mae"].mean(), color="r", linestyle="--", label=f"Mean: {df['mae'].mean():.4f}")
    plt.axvline(df["mae"].median(), color="g", linestyle="--", label=f"Median: {df['mae'].median():.4f}")
    plt.legend()
    ...
    for col in df.columns:
        ...
        if df[col].nunique() < 20:  # Only for categorical data with limited unique values
